...
"""
🔥 SET RESISTANCE SELL ORDERS (FIXED)
...
def set_resistance_sells():
    ...
    try:
        ...
        if btc_balance > 0.0001:
            sell_amount = min(btc_balance * 0.10, 0.001)  # Cap at 0.001 BTC for safety
            ...
            try:
                order = client.limit_order_gtc_sell(
                    client_order_id=f"btc_res_{int(datetime.now().timestamp())}",
                    product_id="BTC-USD",
                    base_size=f"{sell_amount:.8f}",
                    limit_price="110000"
                )
            ...
            print("\n📊 Insufficient BTC balance for sell order")
        
        # Set ETH sell at $4,400 (15% of holdings)
        if eth_balance > 0.001:
            sell_amount = min(eth_balance * 0.15, 0.05)  # Cap at 0.05 ETH for safety
            ...
            try:
                order = client.limit_order_gtc_sell(
                    client_order_id=f"eth_res_{int(datetime.now().timestamp())}",
                    product_id="ETH-USD",
                    base_size=f"{sell_amount:.6f}",
                    limit_price="4400"
                )
            ...
            print("\n📊 Insufficient ETH balance for sell order")
        ...
        if orders_placed:
            ...
            print("No orders placed - check balances")
            print("\n💡 Alternative: Set manual alerts at:")
            print("  • BTC: $110,000")
            print("  • ETH: $4,400")
        ...
        print(f"\n❌ Error: {e}")
        print("\n💡 Manual order setup recommended:")
        print("  1. Set BTC limit sell at $110,000")
        print("  2. Set ETH limit sell at $4,400")
        print("  3. Use 10-15% of holdings")
