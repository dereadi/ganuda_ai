# JR-TEG-1029-N00: Email Triage Wire — Real-Time Urgent Email Alerts

**Task**: Build `/ganuda/services/chief_pa/email_triage_wire.py`
**Priority**: 3
**Assigned**: Software Engineer Jr.

## Context

The Chief PA already has a daily_briefing.py that queries email_classifications
on bluefin for the morning digest. That covers batch/daily coverage. This module
is the REAL-TIME component — it polls the DB every 2 minutes for new
classifications and immediately pushes HIGH URGENCY emails to Slack so the Chief
doesn't wait until the next briefing.

Key interfaces:
- `SlackNotifier.from_config(config)` — factory in slack_notify.py
- `SlackNotifier.send_message(text, category="email-triage", urgent=True)` — for urgent push
- DB table `email_classifications` on bluefin with columns: `id`, `sender_domain`, `subject_preview`, `urgency_score`, `classification`, `classified_at`
- Config from `/ganuda/services/chief_pa/config.yaml` — `database` section for DB, `email` section for thresholds

Privacy constraint (Crawdad): No email body. Only subject_preview (truncated), sender_domain, urgency_score.

## Step 1: Create email_triage_wire.py

Create `/ganuda/services/chief_pa/email_triage_wire.py`
...
- [ ] File created at `/ganuda/services/chief_pa/email_triage_wire.py`
- [ ] Imports psycopg2 with graceful fallback (same pattern as daily_briefing.py)
- [ ] Uses `SlackNotifier.send_message(text, category="email-triage", urgent=True)` for urgent emails
- [ ] Checks `information_schema.tables` for email_classifications existence (no crash if missing)
- [ ] State persisted to `~/.config/chief_pa/email_triage_state.json`
- [ ] No email body stored or transmitted (Crawdad privacy constraint)
- [ ] Signal handling for graceful shutdown
- [ ] Config-driven thresholds from config.yaml `email` section
