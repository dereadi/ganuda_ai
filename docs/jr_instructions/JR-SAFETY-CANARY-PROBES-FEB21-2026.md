# JR Instruction: Safety Canary Probe System

**Task**: SAFETY-CANARY-001
**Priority**: P1
**Kanban**: #1858 (5 SP)
**use_rlm**: false
**assigned_jr**: Software Engineer Jr.
**Council Vote**: PROCEED WITH CAUTION (0.86 confidence, 1.0 agreement)
**Long Man Phase**: BUILD (Phase 1 of 5 — Alignment Collapse Sprint)

## Context

Research from Princeton (Springer et al., arXiv 2602.15799) shows that safety alignment is geometrically fragile — fine-tuning on benign tasks can destroy safety guardrails following a quartic scaling law. The Council identified that the federation has NO automated monitoring of base model refusal behavior. If the model's safety degrades (through quantization artifacts, updates, or future fine-tuning), nothing catches it.

This script runs periodically on redfin, sends known-harmful test prompts to the gateway, verifies the model refuses appropriately, logs results to thermal_memory_archive, and sends Telegram alerts if refusal rate drops below threshold.

The script must run on redfin using `/ganuda/home/dereadi/cherokee_venv/bin/python3`.

## Changes

### Change 1: Create the safety canary probe script

Create `/ganuda/scripts/safety_canary.py`
...
```text
# Dry run to verify probes are configured
/ganuda/home/dereadi/cherokee_venv/bin/python3 /ganuda/scripts/safety_canary.py --dry-run

# Single run — exit code 0 if refusal rate above threshold, 1 if below
/ganuda/home/dereadi/cherokee_venv/bin/python3 /ganuda/scripts/safety_canary.py

# Daemon mode (for systemd later — TPM will create .service file)
/ganuda/home/dereadi/cherokee_venv/bin/python3 /ganuda/scripts/safety_canary.py --daemon
```

Expected output: All 7 probes should show `[REFUSED]`, refusal rate 100%, no Telegram alert fired.

## Notes

- The `.service` file for systemd deployment will be created by the TPM (executor blocks `.service` files).
- DB_PASS must be available as environment variable or will be read from EnvironmentFile in systemd.
- Probes are intentionally standard safety evaluation prompts. The model SHOULD refuse all of them.
- If any probe gets a compliance response, that's the signal that something has changed.
