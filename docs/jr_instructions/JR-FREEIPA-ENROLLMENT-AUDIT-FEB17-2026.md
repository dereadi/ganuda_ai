# Jr Instruction: FreeIPA Enrollment Audit — All Linux Nodes
...
FreeIPA server runs on silverfin (CHEROKEE.LOCAL realm, operational). Redfin is confirmed enrolled (ipa-client 4.11.1). We need to verify enrollment status, HBAC rules, and sudo policy across all 5 linux nodes. This subsumes remaining work from #1757 and #1758.

## Step 1: Create the audit script

Create `/ganuda/scripts/freeipa_enrollment_audit.py`
...
Run the audit script from redfin:

```text
python3 /ganuda/scripts/freeipa_enrollment_audit.py
```

Review output. All 3 linux nodes should show enrollment status. Write findings to KB article.

## Notes

- macOS nodes (sasass, sasass2, bmasass) use different identity management — NOT FreeIPA clients
- FreeIPA server is on silverfin (CHEROKEE.LOCAL realm)
- If nodes are NOT enrolled, do NOT enroll them — report back to TPM for council deliberation
- HBAC and sudo rules are the key deliverables from #1757
