# JR Instruction: Create regenerate_blog_index.py

**Task ID**: REGENERATE-BLOG-INDEX
**Priority**: 3
...
`publish_web_content.py` imports `regenerate_blog_index` (line 18) but the module does not exist. Every blog post publish prints a warning. The blog index (`/ganuda/www/ganuda.us/blog/index.html`) is currently hand-maintained — new posts must be manually added to the HTML.

This script scans blog HTML files, extracts metadata from HTML tags, sorts by date (newest first), generates a styled index page matching the existing design, and publishes it to the `web_content` table for materialization.

Council Vote: #df0c89c957512c88 — unanimous agreement on core scan/parse/generate. RSS, tags, reading time deferred to future sprint.
...
1. Run standalone: `python3 /ganuda/scripts/regenerate_blog_index.py`
2. Should output: `Wrote /ganuda/www/ganuda.us/blog/index.html (N bytes)` and `Blog index regenerated with N post(s).`
3. Open generated index.html — should list all blog posts sorted newest first
4. Test via publish_web_content.py: `python3 /ganuda/scripts/publish_web_content.py /ganuda/www/ganuda.us/blog/natural-falls-ozark-bluffs-camping.html` — should auto-regenerate index without warnings
5. Verify styling matches existing blog index (navy/gold/teal palette, same layout)

## Files Created

- `/ganuda/scripts/regenerate_blog_index.py`
