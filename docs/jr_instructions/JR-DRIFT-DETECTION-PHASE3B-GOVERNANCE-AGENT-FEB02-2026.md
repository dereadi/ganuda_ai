# Jr Instruction: Drift Detection Phase 3B — Governance Agent

**Task:** JR-DRIFT-PHASE3B-GOVERNANCE
**Priority:** P1
**Assigned:** Software Engineer Jr.
**Depends On:** JR-DRIFT-PHASE1A-SQL, JR-DRIFT-PHASE2B-COHERENCE
...
Create an always-on governance monitoring daemon that:
1. Collects Council, Jr executor, thermal memory, and circuit breaker metrics every 30 minutes
2. Evaluates anomaly detection rules against collected metrics
3. Fires severity-graded Telegram alerts when thresholds are breached
4. Stores all metrics in the `drift_metrics` table for trend analysis
5. Exposes a `/drift-status` Telegram command for on-demand status checks

## Prerequisites

- `drift_metrics` table exists (Phase 1A migration)
- `specialist_health` table exists (Phase 1A migration)
- `council_votes` table exists
- `jr_tasks` table exists
- `thermal_memory_archive` has `content_checksum`, `staleness_flagged`, `sacred_pattern`, `freshness_score` columns (Phase 1A + 2A)
- Circuit breaker state function available from drift_detection (Phase 2B)

## Step 1: Create Governance Agent Daemon

**Create:** `/ganuda/daemons/governance_agent.py`

```bash
cat > /ganuda/daemons/governance_agent.py << 'PYEOF'
#!/usr/bin/env python3
"""
...
if __name__ == '__main__':
    main()
PYEOF
chmod +x /ganuda/daemons/governance_agent.py
```

## Step 2: Add `/drift-status` Command to Telegram Chief

**File:** `/ganuda/telegram_bot/telegram_chief.py`

**Deferred.** /drift_status command to be added to telegram_chief.py in a future pass.

## Step 3: Create systemd Service File

**Create:** `/ganuda/scripts/systemd/drift-governance.service`

```bash
cat > /ganuda/scripts/systemd/drift-governance.service << 'SVCEOF'
[Unit]
Description=Drift Governance Agent - Always-On Monitoring
...
ExecStart=/usr/bin/python3 /ganuda/daemons/governance_agent.py
Restart=always
RestartSec=60
...
sudo cp /ganuda/scripts/systemd/drift-governance.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable drift-governance.service
sudo systemctl start drift-governance.service
sudo systemctl status drift-governance.service
```

## Validation

### Test 1: Single Governance Cycle

```bash
cd /ganuda && python3 -c "
from daemons.governance_agent import collect_metrics, check_alerts
metrics = collect_metrics()
...
for k, v in sorted(metrics.items()):
    if isinstance(v, (int, float)):
        ...
        print(f'  {k}: {type(v).__name__} ({len(v) if hasattr(v, \"__len__\") else \"?\"} items)')
print()
alerts = check_alerts(metrics)
...
```bash
# After first run, state file should have baseline sacred count
cat /ganuda/daemons/.governance_state.json
```
...
```bash
sudo systemctl status drift-governance.service
journalctl -u drift-governance.service --since "5 minutes ago" --no-pager
```
...
/drift_status
# (Deferred — command not yet added to telegram_chief.py)
```

## Architecture Notes

- **Metric collectors are isolated:** Each collector (council, jr, memory, circuit breaker) has its own try/except. If one fails, the others still run.
- **Integrity check is expensive:** SHA-256 recomputation of all memory content runs every 6th cycle (3 hours) to avoid DB load.
- **Sacred count is monotonic:** The baseline ratchets upward. New sacred memories update the baseline. Only a DECREASE (deletion or corruption) triggers a CRITICAL alert.
- **State file at** `/ganuda/daemons/.governance_state.json` persists the cycle counter, expected sacred count, and last run timestamp across daemon restarts.
- **Alert delivery fallback:** If direct Telegram API fails, falls back to `alert_manager.py`.
