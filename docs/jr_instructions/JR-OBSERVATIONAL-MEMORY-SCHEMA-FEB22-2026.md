# Jr Instruction: Observational Memory Architecture — Phase 1 Schema

**Task ID:** OBSMEM-SCHEMA
**Kanban:** #1855
**Priority:** 3
**Assigned:** Software Engineer Jr.
**Council Votes:** #c064bf44b624b56f, #f0ff4f4e44416d6d (0.875 PROCEED)
**Constraint:** Raw thermal memories NEVER touched. Parallel layer only.

---

## Overview

Create the Observational Memory schema — a parallel layer that stores compressed, temporally-structured observations derived from thermal memories. This is Phase 1 (schema only, no daemon).

---

## Step 1: Create the schema migration script

Create `/ganuda/scripts/migrations/observational_memory_schema.py`
...
Run the migration:
```text
cd /ganuda/scripts/migrations && python3 observational_memory_schema.py
```

Then verify:
```text
psql -h 192.168.132.222 -U claude -d zammad_production -c "\d observational_memory_archive"
psql -h 192.168.132.222 -U claude -d zammad_production -c "\d observational_relationships"
psql -h 192.168.132.222 -U claude -d zammad_production -c "SELECT column_name FROM information_schema.columns WHERE table_name = 'thermal_memory_archive' AND column_name = 'is_observed'"
```
...
- IVFFlat index with lists=50 is appropriate for expected corpus <100K observations
- The three-date temporal model (observed_at, first_occurrence, last_occurrence) enables trend detection
- observation_hash prevents duplicate observations (ON CONFLICT safe)
- Phase 2 (observer daemon) will be a separate Jr instruction
