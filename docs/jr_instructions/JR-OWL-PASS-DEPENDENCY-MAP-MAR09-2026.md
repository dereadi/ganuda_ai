# Jr Instruction: Owl Pass Dependency Map Generator

## Context
Spider request from Longhouse open floor. Council voted PROCEED. Spider: "We need to see the web before we can protect it." Generates a federation-wide service dependency map showing what depends on what, so failures can be traced to impact.

## Task
Create a dependency map generator that queries all nodes and builds a directed graph of service dependencies.

## Create `/ganuda/scripts/dependency_map_owlpass.py`
...
## Acceptance Criteria
- Maps all known federation services and their dependencies as a directed graph
- Checks node reachability (WireGuard preferred, LAN fallback)
- Identifies root services (no dependencies — e.g., PostgreSQL)
- Identifies single points of failure (3+ dependents)
- Ranks services by impact (most dependents first)
- Writes JSON report to `/ganuda/reports/`
- Results stored in thermal memory
- Graph is manually maintained in `DEPENDENCY_GRAPH` dict — Spider can extend it
