# Jr Instruction: Adaptive GPU Power Monitor

**Task ID**: JR-ADAPTIVE-GPU-POWER-MONITOR-FEB11-2026
**Priority**: P1
**Assigned**: Infrastructure Jr.
**Council Vote**: #ba677ef5213772b7 — REVIEW REQUIRED (0.844), 6/7 concerns
**Depends On**: Existing `/ganuda/scripts/gpu_power_monitor.py`

## Context

The Council unanimously rejected fixed 60-second polling. Consensus across all 7 specialists:
- **Idle interval**: 300s (5 minutes) — all 7 agreed
- **Active interval**: 15s — compromise of Gecko/Raven (10s), Turtle (15s), Spider (30s)
- **Overhead budget**: <1% CPU — all 7 agreed
- **Transition trigger**: GPU utilization >40% OR power delta >20% from baseline
- **Return to idle**: 3 consecutive readings below threshold
- **Add CPU monitoring**: Yes, at idle interval only
- **Log all transitions**: Crawdad security requirement

## Step 1: Convert to Long-Running Adaptive Daemon

File: `/ganuda/scripts/gpu_power_monitor.py`

Replace the entire file with an adaptive daemon. The script becomes a persistent process instead of a timer-triggered oneshot.
...
## Step 2: Update Systemd Service (Long-Running Daemon)

The service changes from `Type=oneshot` (timer-triggered) to `Type=simple` (persistent daemon).

**NOTE: .service files require Chief/TPM approval — staged only, not auto-deployed.**

The TPM will create these files directly:
- `/ganuda/scripts/systemd/gpu-power-monitor.service` — updated to Type=simple with Restart=on-failure
- The `.timer` unit is no longer needed (daemon manages its own scheduling)

## Step 3: Install psutil Dependency

The adaptive monitor needs `psutil` for CPU monitoring. This is a pip install command:

```text
# Run on redfin:
/ganuda/venv/bin/pip install psutil
```

## Verification

After deployment, check:
1. Service starts: `sudo systemctl status gpu-power-monitor`
2. Idle mode logging: `journalctl -u gpu-power-monitor -f` — should show 300s intervals
3. Trigger active: Run a GPU workload, confirm switch to 15s intervals
4. Return to idle: Stop workload, confirm 3 readings then back to 300s
5. Mode transitions logged: `SELECT * FROM unified_timeline WHERE event_type = 'power_monitor_mode_change' ORDER BY timestamp DESC LIMIT 5;`
6. DB records: `SELECT event_type, count(*) FROM unified_timeline WHERE source LIKE 'gpu_%' AND timestamp > NOW() - interval '1 hour' GROUP BY event_type;`

## Council Alignment

| Specialist | Concern | How Addressed |
|------------|---------|---------------|
| Gecko | <1% overhead | 300s idle, single persistent DB conn |
| Raven | Adaptive intervals | 300s idle / 15s active with thresholds |
| Spider | 10MB memory, no DB churn | Persistent conn, minimal state |
| Turtle | Monitor shouldn't increase energy | Idle mode extremely light, no CPU poll in active |
| Crawdad | Log transitions | power_monitor_mode_change events in unified_timeline |
| Eagle Eye | Data aggregation | Reduced records in idle mode (power only, no temp/util) |
| Peace Chief | Consensus | All 6 concerns addressed above |
