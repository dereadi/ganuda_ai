# [TEG] Build Tier 2 Deliberation Handler for Graduated Harness - Node n00
...
Build `/ganuda/lib/harness/tier2_deliberation.py` — the Tier 2 (Deliberation) module of the Graduated Harness. This tier provides quick multi-perspective synthesis using 2-3 specialists selected by domain affinity, with majority-vote consensus and diversity scoring (Elisi requirement).

## Council Design References

- Longhouse #b940f09b18605c97 (UNANIMOUS) — Graduated Harness Tiers
- Council Vote #2c2fa88a53817307 (APPROVED) — Spec #5
- DC-6 Gradient Principle: specialization is gravity, not boundary
- Elisi requirement: diversity scoring in every response

## Dependencies

- `/ganuda/lib/harness/core.py` — HarnessRequest, TierResult, StakesLevel
- `/ganuda/lib/harness/config.py` — TierConfig, EndpointConfig, load_harness_config
- `/ganuda/lib/harness/escalation.py` — TierHandler protocol (handle method signature)
- `/ganuda/lib/harness/config.yaml` — tier2 section for endpoint/threshold config
- Pattern reference: `/ganuda/lib/harness/tier1_reflex.py` (handler pattern)
- Pattern reference: `/ganuda/lib/specialist_council.py` (ThreadPoolExecutor, backend dict, LLM call pattern)

## Architecture

- **Specialist Selection**: Keyword-based domain matching. Map query keywords to specialist domains, pick top 2-3.
- **Parallel Execution**: ThreadPoolExecutor for concurrent specialist queries (same pattern as specialist_council.py).
- **Consensus**: Majority vote. 3/3 agree = high confidence, 2/3 = medium, all disagree = low.
- **Diversity Score**: Measure spread of specialist opinions (higher = more diverse viewpoints). Required by Elisi.
- **Latency target**: <500ms p95.
- **Protocol**: Implements TierHandler from escalation.py: `handle(request, prior_results) -> TierResult`

## Instructions

Create `/ganuda/lib/harness/tier2_deliberation.py`
...
1. The file exists at `/ganuda/lib/harness/tier2_deliberation.py`
2. The file imports correctly: `python3 -c "from lib.harness.tier2_deliberation import Tier2Deliberation"` (run from `/ganuda`)
3. The class implements `handle(request, prior_results) -> TierResult` matching the TierHandler protocol
4. Diversity score is included in every TierResult (Elisi requirement)
5. ThreadPoolExecutor is used for parallel specialist queries
6. No hardcoded credentials anywhere in the file
