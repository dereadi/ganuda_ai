# Jr Instruction: Replace Wizard Page (EXPLICIT)

```yaml
task_id: wizard_form_selection_explicit
priority: 1
...
Replace the entire file `/ganuda/vetassist/frontend/app/wizard/page.tsx` with the following content:
...
After replacing, run:
```bash
ls -la /ganuda/vetassist/frontend/app/wizard/page.tsx
head -20 /ganuda/vetassist/frontend/app/wizard/page.tsx
```

The file should start with `'use client';` followed by `import { useState }`.
