# JR Instruction: ATP Counter (Token Economics)

**Task**: ATP-COUNTER-001
**Title**: Create ATP Counter — Token Accounting Wrapper
**Priority**: 2
**Assigned Jr**: Software Engineer Jr.

## Context

Every mitochondrion (LLM) consumes resources and produces tokens. The ATP counter wraps LLM calls to log token consumption to the `token_ledger` table. This gives the federation energy awareness — cost per council vote, daily burn rate, which specialists are expensive vs cheap.

The Duplo composer already logs enzyme invocations. This module provides:
1. A standalone wrapper for non-Duplo LLM calls (council votes, gateway, TPM)
2. Query functions for reading the ledger (daily summary, per-model, per-caller)
3. The council vote integration patch (replace `tokens_used=0` hardcoding)

Depends on: DB migration `duplo_schema.sql` (token_ledger table)

## Files

Create `lib/duplo/atp_counter.py`
...
## Council Vote Integration (Phase 2 — separate Jr task)

The council vote pipeline in `specialist_council.py` currently hardcodes `tokens_used=0` in the api_audit_log INSERT. After ATP counter is deployed, a follow-up Jr task should:

1. Import `count_tokens` from `lib.duplo.atp_counter`
2. In `_query_specialist()`, capture the vLLM response JSON before extracting content
3. Call `count_tokens(response_data, model, f"council.{specialist_id}", "council_vote", metadata={"vote_hash": audit_hash})`
4. Use the returned token count in the audit_log INSERT instead of 0

This is a surgical edit to `specialist_council.py` — do NOT attempt it in this task.

## Verification

1. Import: `python3 -c "from lib.duplo.atp_counter import count_tokens, get_daily_burn; print('OK')"`
2. Manual insert test:
   ```text
   python3 -c "
   from lib.duplo.atp_counter import count_tokens
   r = count_tokens(
   ...
   "
   ```
3. Query: `python3 -c "from lib.duplo.atp_counter import get_daily_burn; print(get_daily_burn(1))"`
