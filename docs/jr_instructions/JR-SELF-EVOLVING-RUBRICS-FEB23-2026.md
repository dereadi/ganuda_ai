# Jr Instruction: Self-Evolving Rubrics — Process Reward Model
...
Create a Process Reward Model evaluator that scores council vote quality over time. Analyzes confidence calibration, concern quality, and consensus accuracy across the last 50 votes.

---

## Step 1: Create the rubric evaluator script

Create `/ganuda/scripts/rubric_evaluator.py`
...
```text
python3 /ganuda/scripts/rubric_evaluator.py
cat /ganuda/reports/rubric-scores.json
```

Expected: JSON report with confidence calibration, concern quality, and consensus accuracy scores.
