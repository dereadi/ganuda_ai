# Jr Instruction: Node Health Dashboard (Traffic Light)

**Task**: Create a node health page at ganuda.us/health.html showing service status for all nodes
**Priority**: 3
**Story Points**: 5
**Epic**: #1974
...
Chief needs a traffic-light view of all nodes and their key services. Script runs on redfin, checks services via SSH or direct HTTP probes, publishes HTML to web_content.

Node map:
- redfin (192.168.132.223): vLLM:8000, gateway:8080, SAG:5000, jr-se.service
- bluefin (192.168.132.222): PostgreSQL:5432, VLM:8090, YOLO:8091, VLM-Adapter:8092
- greenfin (192.168.132.224): Embedding:8003, OpenObserve
- owlfin (192.168.132.170): Caddy web (HTTP 80)
- eaglefin (192.168.132.84): Caddy web (HTTP 80)
- bmasass (192.168.132.21): MLX DeepSeek:8800

## Steps

### Step 1: Create health check script

Create `/ganuda/scripts/generate_health_page.py`
...
1. Run: `cd /ganuda && python3 scripts/generate_health_page.py`
2. Output should show node/service counts
3. Check: page visible at ganuda.us/health.html
