# Jr Instruction: Owl Pass Credential Audit — Fix Hardcoded Secrets

## Context
Crawdad request from Longhouse open floor. Council voted PROCEED. Hardcoded DB password found in VetAssist bluefin copy (main.py). credential-scanner.py exists but only runs on redfin. Need cross-node scan via FreeIPA SSH.

## Task
Fix the known hardcoded credential and scan all nodes for others.

## File: `/ganuda/vetassist/backend/main.py`
