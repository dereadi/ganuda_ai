# Jr Instruction: Add Embedding Diversity Measurement to Council Votes

**Task ID**: COUNCIL-DIVERSITY-001
**Priority**: 3 (medium)
...
Per SD-MoE research (arXiv:2602.12556), MoE experts can converge on shared spectral directions — producing redundant responses instead of genuine diversity. We need to measure whether our council specialists are actually producing diverse responses or converging on the same semantic content with different words. This script analyzes historical council votes by computing cosine similarity between specialist responses in embedding space (BGE-large 1024d on greenfin:8003).

## Step 1: Create Diversity Diagnostic Script

Create `/ganuda/scripts/council_diversity_diagnostic.py`
...
1. Run on redfin: `python3 /ganuda/scripts/council_diversity_diagnostic.py --limit 30`
2. Review output for spectral convergence patterns
3. If mean similarity > 0.85, escalate to council for prompt redesign
4. Store results in thermal memory
