# Jr Instruction: Self-Healing Phase 4 — ARA Callback + Thermal Memory Feedback

**Kanban**: #1781 (Phase 4 of 4)
**Story Points**: 2
**Council Vote**: #1872d8f580eaec28 (PROCEED WITH CAUTION, 0.874)
**Priority**: 17 (RC-2026-02B)
**Dependencies**: Phase 3 (Validation Pipeline) must be complete
**Risk**: LOW — new files only, one SR block to existing ansible.cfg

## Objective

Create the ARA-to-thermal-memory bridge that reads Ansible execution results
from ARA's PostgreSQL store and writes them back to thermal_memory_archive.
This closes the learning loop — every remediation execution becomes knowledge
for future remediations.

Also update ansible.cfg to enable the ARA callback plugin.

## Step 1: Create ARA-to-Thermal-Memory Bridge

Create `/ganuda/ansible/remediation/ara_thermal_bridge.py`
...
## Step 2: Update ansible.cfg with ARA callback configuration

File: `/ganuda/ansible/ansible.cfg`
...
=======
[defaults]
# ARA callback plugin — records all playbook executions
# Install: pip install ara[server]
# Start ARA server: ara-manage runserver 0.0.0.0:8888
callback_plugins = %(VIRTUAL_ENV)s/lib/python3.12/site-packages/ara/plugins/callback
>>>>>>> REPLACE
```
...
# Initialize ARA database (uses SQLite by default, can switch to PostgreSQL later)
ara-manage migrate
# Test ARA server
ara-manage runserver 0.0.0.0:8888 &
# Verify callback works
ARA_API_CLIENT=http ARA_API_SERVER=http://localhost:8888 ansible-playbook playbooks/smoke-test.yml
```
