# JR Instruction: Credential Hygiene Pass
...
Raven (contrarian specialist) flagged during Council deliberation that the federation's DB password appears in plaintext in auto-memory files and potentially in scripts. This is a credential hygiene issue independent of quantum threats — it's a classical security gap.

This task scans for plaintext credentials in /ganuda and produces a remediation report. It does NOT automatically remove credentials (too dangerous) — it identifies them for manual review.

The script must run on redfin using `/ganuda/home/dereadi/cherokee_venv/bin/python3`.

## Changes

### Change 1: Create the credential scanner

Create `/ganuda/scripts/credential_scanner.py`
...
```text
/ganuda/home/dereadi/cherokee_venv/bin/python3 /ganuda/scripts/credential_scanner.py
cat /ganuda/reports/credential_hygiene_feb2026.md
```

Expected: Report listing all files with potential plaintext credentials, with values redacted, and remediation recommendations.

## Notes

- Script is READ-ONLY — does not modify any files.
- Credential values are redacted in the report (first 3 + last 3 chars only).
- Large directories (venvs, node_modules, old data) are skipped for performance.
- Manual review of findings required — false positives expected from example configs.
- .env files are in the allowlist (that is where credentials SHOULD live).
