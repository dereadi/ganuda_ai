# Jr Instruction: Owl Pass Chaos Test — Controlled Node-Failure Simulation

## Context
Coyote request from Longhouse open floor. Council voted PROCEED. Coyote amendment honored: test Fire Guard itself too, not just the services. "The current stability is an illusion until we've stress-tested it." OBSERVE AND REPORT ONLY — no destructive actions.

## Task
Create a chaos test script that systematically stops one service at a time, verifies Fire Guard detects it, then restarts. Reports results.

## Create `/ganuda/scripts/chaos_test_owlpass.py`
...
## Acceptance Criteria
- Script stops each target service one at a time
- Waits 150s (Fire Guard cycle + buffer) and checks if Fire Guard detected the failure
- Restarts service and verifies recovery
- Reports detection rate and recovery rate
- Results stored in thermal memory
- DOES NOT run automatically — manual execution only during maintenance windows
- Uses FreeIPA sudo for all systemctl operations
