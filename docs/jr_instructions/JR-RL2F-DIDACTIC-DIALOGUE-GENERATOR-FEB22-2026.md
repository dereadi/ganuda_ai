# Jr Instruction: RL2F Phase 1 — Didactic Dialogue Generator

**Task ID:** RL2F-DIAL-GEN
**Kanban:** #1881
**Priority:** 80
...
Create a batch script that generates teacher-student dialogues from completed council vote outcomes. For each historical council vote, Qwen 72B plays teacher (with ground truth) and student (reasoning from scratch). Target: 5K dialogues for QLoRA training data.

---

## Step 1: Create the didactic dialogue generator script

Create `/ganuda/scripts/rl2f/didactic_dialogue_generator.py`
...
After creating, test with dry-run:
```text
cd /ganuda/scripts/rl2f
python3 didactic_dialogue_generator.py --dry-run
```

Then generate a small test batch:
```text
python3 didactic_dialogue_generator.py --batch-size 5 --max-dialogues 3
```

Check results:
```text
psql -h 192.168.132.222 -U claude -d zammad_production -c "SELECT id, source_vote_id, turns, quality_score, student_final_correct FROM didactic_dialogues LIMIT 10;"
```
...
- Each dialogue = ~8 vLLM calls (4 student + 4 teacher) × ~300 tokens = ~2400 tokens per dialogue
- 500 dialogues ≈ 1.2M tokens ≈ 3-4 hours on redfin vLLM at 32 tok/s
- Quality score: 0.5 (correct) + 0.3 (no leakage) + 0.2 (full turns) = max 1.0
- Min quality threshold 0.5 filters out dialogues where student never converges
- `included_in_training` stays false until Phase 2 QLoRA picks them up
