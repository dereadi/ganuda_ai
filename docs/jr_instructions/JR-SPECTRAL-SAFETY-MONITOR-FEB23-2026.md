# Jr Instruction: Spectral Safety Monitor — SVD on LoRA Deltas

**Task ID:** SPECTRAL-SAFETY
**Kanban:** #1860
**Priority:** 3
...
Create a spectral safety monitor that performs SVD on LoRA weight deltas to detect anomalous weight changes that could indicate catastrophic forgetting or adversarial fine-tuning.

---

## Step 1: Create the spectral safety monitor

Create `/ganuda/scripts/safety/spectral_safety_monitor.py`
...
```text
mkdir -p /tmp/test-adapter
python3 -c "import numpy as np; np.save('/tmp/test-adapter/test_layer.npy', np.random.randn(64, 32))"
python3 /ganuda/scripts/safety/spectral_safety_monitor.py /tmp/test-adapter
cat /ganuda/reports/spectral-safety-report.json
```
