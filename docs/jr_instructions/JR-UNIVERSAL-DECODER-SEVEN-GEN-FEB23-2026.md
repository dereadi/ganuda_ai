# Jr Instruction: Universal Decoder — Seven Generations Compression

**Task ID:** UNIV-DECODER
**Kanban:** #530
**Priority:** 4
...
Create a universal decoder module with Cherokee-specific codecs for long-term knowledge preservation. Ensures compressed data remains readable 100+ years. Implements modular codec architecture.

---

## Step 1: Create the universal decoder module

Create `/ganuda/services/universal_decoder.py`
...
```text
python3 -c "
from universal_decoder import encode, decode
text = 'Seven Generations preservation test'
blob = encode(text, 'markdown')
assert decode(blob) == text
print(f'Markdown: {len(text)} -> {len(blob)} bytes, round-trip PASS')
records = [{'id': 1, 'content': 'test', 'temp': 75}]
blob2 = encode(records, 'thermal')
assert decode(blob2) == records
print(f'Thermal: round-trip PASS')
"
```
