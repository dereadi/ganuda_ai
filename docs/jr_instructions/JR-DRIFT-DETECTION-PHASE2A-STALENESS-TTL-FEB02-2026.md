# Jr Instruction: Drift Detection Phase 2A — Staleness TTL Daemon

**Task:** JR-DRIFT-PHASE2A-STALENESS
**Priority:** P1
...
Create a staleness scoring daemon that:
1. Computes freshness scores for all thermal memories based on age, access patterns, and domain
2. Flags stale memories (freshness < 0.2) so they're deprioritized in reads
3. Modifies `get_recent_memories()` in rlm_bootstrap.py to deprioritize stale memories

## Step 1: Create Staleness Scorer Daemon

**Create:** `/ganuda/daemons/staleness_scorer.py`

```bash
cat > /ganuda/daemons/staleness_scorer.py << 'PYEOF'
#!/usr/bin/env python3
"""
...
if __name__ == '__main__':
    main()
PYEOF
chmod +x /ganuda/daemons/staleness_scorer.py
```

## Step 2: Modify RLM Bootstrap to Deprioritize Stale Memories

**File:** `/ganuda/lib/rlm_bootstrap.py`

**Applied by TPM.** rlm_bootstrap.py restored and updated with staleness filter.

## Validation

```bash
# Test staleness scorer directly
cd /ganuda && python3 -c "
from daemons.staleness_scorer import run_staleness_cycle
result = run_staleness_cycle()
print(f'Result: {result}')
"

# Verify stale memories were flagged
psql -h 192.168.132.222 -U claude -d zammad_production -c "
SELECT COUNT(*) AS stale_count,
       AVG(freshness_score)::numeric(4,2) AS avg_freshness
FROM thermal_memory_archive
WHERE staleness_flagged = true;
"

# Verify RLM bootstrap excludes stale memories
cd /ganuda && python3 -c "
from lib.rlm_bootstrap import ThermalBootstrap
tb = ThermalBootstrap()
memories = tb.get_recent_memories()
print(f'Got {len(memories)} non-stale memories')
for m in memories[:3]:
    print(f'  freshness={m.get(\"freshness\", \"N/A\")}, sacred={m[\"sacred\"]}')
tb.close()
"
```
