# Jr Instruction: RL2F Phase 2 — QLoRA Fine-Tune Setup
...
Create the QLoRA fine-tuning script for Qwen2.5-72B-Instruct-AWQ using didactic dialogues. Uses PEFT/bitsandbytes for 4-bit QLoRA on the RTX PRO 6000 (96GB VRAM). Training window: overnight (11pm-7am). bmasass DeepSeek-R1 as fallback during training.

This script DOES NOT run the training — it creates the infrastructure. TPM will schedule overnight runs after safety canary baseline is established.

---

## Step 1: Create the QLoRA training script

Create `/ganuda/scripts/rl2f/qlora_finetune.py`
...
```text
cd /ganuda/scripts/rl2f
python3 qlora_finetune.py --dry-run
```
...
- QLoRA: 4-bit quantized LoRA, trains only adapter weights (~0.1% of total parameters)
- Target modules: all attention projections + gate/up/down (MLP layers)
- LoRA rank 16, alpha 32 (2x scaling — moderate capacity)
- Gradient checkpointing + accumulation = fits in 96GB VRAM
- Safety canary runs BEFORE and AFTER training automatically
- Regression benchmark runs BEFORE and AFTER training automatically
- 5% regression threshold from regression_benchmark.py is the hard gate
- Adapter saved separately — vLLM multi-LoRA serving can hot-swap
- Training batch tracked in didactic_dialogues.training_batch for audit trail
