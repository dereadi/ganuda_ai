# Jr Instruction: RAG Phase 2d — Vectara open-rag-eval Framework

**Task**: Add automated RAG quality evaluation using Vectara's open-rag-eval
**Council Vote**: #33e50dc466de520e (RC-2026-02C, 25 pts, 3/7)
**Kanban**: #1769
**Priority**: 4
...
We need to measure RAG quality to know if our changes (cross-encoder, HyDE) are actually improving retrieval. Vectara's `open-rag-eval` is an open-source framework for evaluating RAG systems.

This task creates a test harness with golden Q&A pairs drawn from our thermal memory, and evaluates retrieval quality using nDCG, recall, and precision metrics.

## Step 1: Create RAG evaluation script

Create `/ganuda/scripts/rag_eval.py`
...
Run the baseline evaluation BEFORE deploying Phase 2b/2c changes:

```text
cd /ganuda && python3 scripts/rag_eval.py
```

Save the baseline numbers. Then after deploying cross-encoder and HyDE, run again and compare. Track improvement over time in `/ganuda/reports/rag_eval/`.

## Success Criteria

- [ ] 8 golden test cases covering key tribal knowledge domains
- [ ] Precision@5, Recall, MRR metrics calculated per query
- [ ] Results saved to `/ganuda/reports/rag_eval/` as JSON
- [ ] Baseline evaluation runs without errors
- [ ] Reports dir created and accessible
