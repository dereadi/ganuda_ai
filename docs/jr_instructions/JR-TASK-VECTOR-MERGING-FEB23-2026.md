# Jr Instruction: Task Vector Merging Evaluation

**Task ID:** TASK-VEC-MERGE
**Kanban:** #1862
**Priority:** 3
...
Create an evaluation script for TIES merging (trim, elect sign, disjoint merge) of LoRA adapters. Compares merged vs individual adapter performance on test prompts.

---

## Step 1: Create the task vector evaluation script

Create `/ganuda/scripts/task_vector_eval.py`
...
```text
mkdir -p /tmp/adapter-a /tmp/adapter-b
python3 -c "import numpy as np; np.save('/tmp/adapter-a/layer1.npy', np.random.randn(64,32)); np.save('/tmp/adapter-b/layer1.npy', np.random.randn(64,32))"
python3 /ganuda/scripts/task_vector_eval.py --adapter1 /tmp/adapter-a --adapter2 /tmp/adapter-b
```
