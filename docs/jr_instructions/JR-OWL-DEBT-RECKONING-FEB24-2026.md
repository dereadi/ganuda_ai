# Jr Instruction: Owl — Debt Reckoning Daemon

**Task ID:** OWL-DEBT-RECKONING
**Kanban:** NEW (create: title "Owl — Debt Reckoning Daemon", priority 1, SFP 65, story points 8)
**Priority:** 1
...
Create a script that looks backward at what the federation shipped and verifies it actually works. Named for the Owl, who sees what others miss in the dark.

The script:
1. Queries `jr_work_queue` for recently completed tasks
2. For each completed task, checks if the files it created actually exist
3. For scripts, does a syntax check (python3 -c "import ast; ast.parse(open(f).read())")
4. For DB table creation tasks, verifies the table exists and has rows
5. Checks for common failure patterns: empty files, duplicate function defs, import errors
6. Scores each task as VERIFIED / DEGRADED / BROKEN
7. Stores a debt reckoning report in thermal_memory_archive
8. Prints a summary to stdout and saves JSON report

Two new files:
1. `/ganuda/scripts/owl_debt_reckoning.py` — the main reckoning script
2. No dependencies beyond psycopg2 and stdlib

---

## Step 1: Create the Owl debt reckoning script

Create `/ganuda/scripts/owl_debt_reckoning.py`
...
1. Syntax check: `python3 -c "import ast; ast.parse(open('/ganuda/scripts/owl_debt_reckoning.py').read()); print('OK')"`
2. Dry run on recent tasks: `python3 /ganuda/scripts/owl_debt_reckoning.py --days 3 --dry-run`
3. Full run: `python3 /ganuda/scripts/owl_debt_reckoning.py --days 3`
4. Check report: `python3 -m json.tool /ganuda/reports/owl_debt_reckoning.json | head -30`

---

## Notes

- Named for the Owl, who sees what others miss in the dark. Owl watches the trail behind us.
- Complements Peace Eagle (Council Curiosity), who watches the horizon ahead.
- Anti-80/20 Principle: sprint hard but stop to verify. Ship capability, not just code.
- The `BROKEN` score means a task marked complete actually didn't deliver its artifacts.
- `DEGRADED` means partial delivery — some steps failed but something shipped.
- Exit code 1 if any BROKEN tasks found, so it can be used in CI/cron.
- Idempotent — thermal memory insert uses ON CONFLICT DO NOTHING on memory_hash.
