# Jr Instruction: Shadow Council — bmasass Thermal Memory Sync
...
Create a daemon that periodically syncs thermal memories to bmasass (M4 Max 128GB, DeepSeek-R1-32B), runs independent deliberation on a subset of recent council questions, and detects divergence between primary (redfin/Qwen-72B) and shadow (bmasass/DeepSeek-R1) council conclusions. Divergence signals potential model bias.

---

## Step 1: Create the shadow council sync daemon

Create `/ganuda/daemons/shadow_council_sync.py`
...
Test shadow council connectivity:
```text
curl -s http://192.168.132.21:8800/v1/chat/completions -X POST \
  -H 'Content-Type: application/json' \
  -d '{"model": "mlx-community/DeepSeek-R1-Distill-Qwen-32B-4bit", "messages": [{"role": "user", "content": "hello"}], "max_tokens": 50}'
```

Run comparison:
```text
cd /ganuda/daemons
python3 shadow_council_sync.py --compare --sample-size 5
```
...
- Shadow council = bmasass M4 Max 128GB running DeepSeek-R1-32B-4bit on MLX
- Primary council = redfin Qwen2.5-72B-Instruct-AWQ on vLLM
- Divergence = different recommendation (PROCEED vs BLOCK) on same question
- 30% divergence threshold triggers Telegram alert
- Thermal memory logging with sacred_pattern=true on high divergence
- bmasass timeout 120s (MLX on Apple Silicon is slower than vLLM on GPU)
- Daemon mode syncs every 6 hours by default
- Memory sync is read-only (no writes to bmasass DB)
