# Jr Instruction: Owl Pass Council Vote Query Tool

## Context
Turtle request from Longhouse open floor. Council voted PROCEED. Turtle: "We need to be able to find our own decisions." Currently council_votes has 28 columns but no easy query interface. Dawn Mist only looks at unfinalized votes. Need a CLI tool to search, filter, and display past votes.

## Task
Create a council vote query CLI that searches by audit_hash, keyword, specialist, date range, confidence threshold, and vote mode.

## Create `/ganuda/scripts/council_vote_query.py`
...
## Acceptance Criteria
- CLI with argparse — queryable by hash, keyword, specialist, date range, confidence, mode, status
- Partial hash matching (ILIKE) for quick lookups
- Keyword search across question, recommendation, and consensus text
- Summary statistics when returning multiple results (avg confidence, concern totals, mode distribution)
- Verbose mode shows full consensus, TPM comments, specialist list, latency
- Default: last 20 votes if no filters specified
- No mutations — read-only query tool
- Uses federation DB credentials from secrets.env
