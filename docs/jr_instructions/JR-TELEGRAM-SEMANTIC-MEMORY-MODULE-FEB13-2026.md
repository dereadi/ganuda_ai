# Jr Instruction: Telegram Bot Semantic Memory Module

**Task**: Create shared semantic memory search module for telegram bots
**Council Vote**: #7ab07bfbd92c70b4 (0.872 confidence, APPROVED)
**Kanban**: #1779
**Priority**: 1 (P0 — foundation for all bot enhancements)
**Assigned Jr**: Software Engineer Jr.

## Context

Both telegram bots (@derpatobot and @ganudabot) need access to the tribe's 80K+ thermal memories via semantic search. Currently the bots use ILIKE text search (low quality) or an outdated MiniLM model (wrong dimensions).

The federation runs BGE-large-en-v1.5 (1024d) on greenfin:8003 and thermal_memory_archive has pgvector embeddings indexed with IVFFlat.

## Step 1: Create Shared Module

Create `/ganuda/telegram_bot/tribe_memory_search.py`
...
```text
cd /ganuda/telegram_bot && python3 -c "from tribe_memory_search import semantic_search; print('OK')"
```
