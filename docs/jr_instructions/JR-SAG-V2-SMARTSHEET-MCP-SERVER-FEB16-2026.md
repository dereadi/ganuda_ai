# Jr Instruction: SAG v2 Smartsheet Resource Management MCP Server

**Task ID**: SAG-V2-MCP-SMARTSHEET
**Priority**: 2
...
## Objective
Create an MCP server wrapping the Smartsheet Resource Management API (formerly 10000ft Plans) at api.rm.smartsheet.com/api/v1/. This server provides native availability endpoints that complement the Productive MCP server.

**IMPORTANT**: Use v2 API endpoints only. Legacy 10000ft APIs were sunset Feb 9, 2026.

## Step 1: Create smartsheet_mcp.py

Create `/ganuda/services/sag-v2/mcp_servers/smartsheet_mcp.py`
...
## Manual Steps (TPM)
- Add `SMARTSHEET_RM_TOKEN` to `/ganuda/config/secrets.env` once Russell provides credentials
- Test: `echo '{"jsonrpc":"2.0","method":"tools/list","id":1}' | python smartsheet_mcp.py`
