# Jr Instruction: Fix Hardcoded Credentials in thermal_health.py
...
```text
python3 -c "import sys; sys.path.insert(0,'/ganuda'); exec(open('/ganuda/services/llm_gateway/thermal_health.py').read().split('def ')[0]); print('OK: uses secrets_loader')"
```
