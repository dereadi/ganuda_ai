# Jr Instruction: Self-Healing Phase 3 — Validation Pipeline + Crawdad Integration

**Kanban**: #1781 (Phase 3 of 4)
**Story Points**: 3
**Council Vote**: #1872d8f580eaec28 (PROCEED WITH CAUTION, 0.874)
**Priority**: 16 (RC-2026-02B)
**Dependencies**: Phase 2 (Remediation Engine) must be complete
**Risk**: LOW — new files only

## Objective

Build the validation pipeline that checks staged remediation playbooks with
ansible-lint, runs --check dry-run, and submits to the council for Crawdad's
security review. No playbook executes without passing all three gates.

## Step 1: Create Validation Pipeline Script

Create `/ganuda/ansible/remediation/validator.py`
...
## Step 2: Create Crawdad Security Review Prompt

Create `/ganuda/ansible/remediation/crawdad_review_prompt.py`
...
Should this playbook be approved for TPM review and potential execution?
Respond PROCEED if safe, BLOCK if security concerns exist."""
```
...
mkdir -p /ganuda/ansible/approved /ganuda/ansible/rejected
```
