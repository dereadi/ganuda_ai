# Jr Instruction: Coyote Phase 1 — Short Context Thesis Test Script

**Task**: Create test script that compares council quality at 2K context vs 32K context
**Priority**: 5
**Story Points**: 5
**Council Vote**: 5ad7577ba1d7685a (Coyote challenged short-context thesis)

## Context

Coyote challenged the thesis that governance + RAG can replace large conventional context windows. This script runs the experiment: same questions through the council at 2K max context (RAG-only retrieval) vs 32K context, then compares response quality.

Coyote owns this experiment. If short-context fails, it saves us from a bad bet on 405B sharding. If it holds, the path opens with full consent.

## Steps

### Step 1: Create the test script

Create `/ganuda/scripts/coyote_context_thesis_test.py`
...
1. Syntax check: `python3 -c "import ast; ast.parse(open('/ganuda/scripts/coyote_context_thesis_test.py').read())"`
2. Dry run (will need DB + embedding service): `cd /ganuda && python3 scripts/coyote_context_thesis_test.py`
3. Review output in `/tmp/coyote_context_test_results.json`
