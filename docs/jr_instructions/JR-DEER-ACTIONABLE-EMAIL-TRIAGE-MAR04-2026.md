# Deer Actionable Email Triage — Smart Classification + Telegram Alerts

## Context
Chief missed a scheduled call with Tanya Pina (OneChronos) because the email containing
the calendar invite sat in inbox without surfacing urgently. The existing gmail_api_daemon.py
has keyword-based analysis that would never catch "let's schedule a call" or "I sent you
a calendar invite" as actionable.

Fix: Replace the primitive keyword analyzer with LLM-backed classification via our own
gateway (redfin:8080), add ACTIONABLE category with Telegram alert, and run the daemon
as a service.

Kanban: #1960 | Long Man phase: BUILD
