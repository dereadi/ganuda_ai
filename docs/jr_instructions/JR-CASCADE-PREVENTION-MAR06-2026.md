# Jr Instruction: Cascade Prevention Circuit Breakers (Eagle Eye -> Feature)

**Task**: Add circuit breakers between SRE scales to prevent cascading failures
**Priority**: 4 (Depends on Signal Contracts + Misalignment Monitor)
...
Eagle Eye's concern on DC-11: "If one scale fails, does the failure cascade?"

Circuit breakers prevent failures at one scale from propagating to others:
- If Tier 1 reflex starts failing, Tier 2 should absorb load, not also fail
- If Claude backend goes down, traffic routes to local models, not to errors
- If the valence queue overflows, reactions still fire (just unmonitored)
- If a node goes offline, federation reroutes, not crashes

DC-10: Each timescale has autonomy. A broken spinal cord doesn't stop the heart.
DC-11: Circuit breakers are the conserved pattern at boundary crossings.

## Steps

### Step 1: Create the circuit breaker module

Create `/ganuda/lib/harness/circuit_breaker.py`
...
```text
cd /ganuda && python3 -c "
from lib.harness.circuit_breaker import CircuitBreaker, BreakerState, create_default_breakers

# Test basic breaker lifecycle
b = CircuitBreaker(name='test', failure_threshold=3, recovery_timeout=0.1)
...
print(f'Tripped after 3 failures: {b.state.value}')

# Wait for recovery timeout
import time; time.sleep(0.2)
assert b.allow_request()  # Should transition to HALF_OPEN
assert b.state == BreakerState.HALF_OPEN
print(f'Recovery attempt: {b.state.value}')

# Successful recovery
b.record_success()
assert b.state == BreakerState.CLOSED
print(f'Recovered: {b.state.value}')

# Test registry
registry = create_default_breakers()
print(f'Registry: {len(registry.status_all())} breakers')
print(f'Any open: {registry.any_open()}')
...
- Claude backend has lowest failure threshold (3) and longest recovery (120s) — external dependency.
- Valence evaluator has highest threshold (10) — it's background, not hot path.
- Half-open state allows limited testing before full recovery.
- Manual reset requires operator/Council — prevents auto-recovery from masking real issues.
- The breaker registry will be wired into the EscalationEngine in a follow-up task.
