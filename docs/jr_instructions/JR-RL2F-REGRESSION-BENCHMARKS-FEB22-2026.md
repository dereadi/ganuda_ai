# Jr Instruction: RL2F Regression Benchmark Suite

**Task ID:** RL2F-BENCH
**Kanban:** #1883
**Priority:** 3
**Assigned:** Software Engineer Jr.
**Council Vote:** #33bc6cc45d77f038 (Coyote binding condition)

---

## Overview

Create a regression benchmark script that establishes baselines BEFORE any QLoRA fine-tuning. Three test suites:

1. **Council Vote Accuracy**: Can the model synthesize multi-specialist input correctly?
2. **Cherokee Values Alignment**: Does the model maintain Seven Generations / Two Wolves / Coyote values?
3. **VetAssist CFR Correctness**: Given a condition, does the model retrieve the right CFR reference?

Hard rollback threshold: **>5% regression on any suite blocks Phase 2 QLoRA training.**

---

## Step 1: Create the regression benchmark script

Create `/ganuda/scripts/rl2f/regression_benchmark.py`
...
Run baseline mode:
```text
cd /ganuda/scripts/rl2f && python3 regression_benchmark.py --mode baseline
```

Expected: Runs all 3 suites (12 test cases), prints scores, saves baseline to `/ganuda/reports/rl2f_benchmarks/baseline.json`.

---

## Notes

- Uses `temperature=0.3` for reproducibility across runs
- CFR test cases cover the top 5 most common VA disability categories
- Comparison mode exits with code 1 on regression (can be used in CI gates)
- No LLM-as-judge — uses keyword signal matching for deterministic scoring
