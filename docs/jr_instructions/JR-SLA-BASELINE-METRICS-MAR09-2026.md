# Jr Instruction: Measure — SLA Baseline Metrics (Governance Response Time)

**Task #1206**
**Date:** 2026-03-09
**Priority:** 2 (Required before SLA Framework #1207)
**TPM:** Claude Opus

## Context

Before we can define SLAs (Task #1207), we need to know what the federation actually delivers today. We have no baseline measurements for council vote latency, Jr task throughput, thermal write speed, or gateway response time. You cannot promise what you have not measured. Sam Walton counted every penny — we measure every millisecond.

## CRITICAL: Database Schema Reference

Use these EXACT column/table names (do NOT guess):
- **council_votes**: vote_id (PK), audit_hash, question, recommendation, confidence, voted_at, tpm_approved_at
- **jr_work_queue**: id, title, status, created_at, updated_at, completed_at, assigned_jr
- **thermal_memory_archive**: id, original_content (NOT "content"), temperature_score (NOT "temperature"), created_at, memory_hash, sacred_pattern
- **Gateway health**: GET http://localhost:8080/health (redfin local)

## Task

Write a script at `/ganuda/scripts/sla_baseline_metrics.py` that queries the bluefin database to measure current federation performance baselines across 5 key metrics. Output results as a formatted table to stdout and optionally write a summary to `/ganuda/docs/business/SLA-BASELINE-METRICS-MAR09-2026.md`.

## Steps

### Step 1: Create the script file

Create `/ganuda/scripts/sla_baseline_metrics.py` with the following content:
...
- Council vote time measures creation-to-resolution (includes async specialist deliberation)
...
Verify that all 5 metric sections produce output (even if some show N/A due to missing data).
