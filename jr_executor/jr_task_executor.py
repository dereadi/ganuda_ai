...
CODE_SAFE_WRITE_PATHS = [
    ...
    '/ganuda/scripts/',
    '/ganuda/docs/',
    '/ganuda/vetassist/',  # VetAssist application code
    '/ganuda/www/',        # Web content (landing pages)
...
class JrTaskExecutor:
    ...
    def get_orphan_tasks(self) -> List[dict]:
        ...
        Council Wisdom: "The rabbit who only looks for the hawk above misses the snake below."
        """
        try:
            conn = self._get_connection()
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    ...
                    LEFT JOIN jr_task_bids b ON t.task_id = b.task_id
                        AND b.bid_at > NOW() - INTERVAL '60 seconds'
                    WHERE t.status = 'open'
    ...
    def cleanup_stale_tasks(self):
        ...
        This handles crashed agents and hung tasks.
        """
        try:
            conn = self._get_connection()
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
    ...
    def _call_llm_routed(self, prompt: str, max_tokens: int = 4000) -> str:
        """
        Call LLM Gateway for code generation.
        Uses /v1/chat/completions with Qwen2.5-Coder-32B-AWQ model.
        """
        try:
            response = requests.post(
                f"{GATEWAY_URL}/v1/chat/completions",
                ...
                json={
                    "model": "auto",  # Routed based on task type
                    "messages": [{"role": "user", "content": prompt}],
                    "max_tokens": max_tokens,
                    "temperature": 0.1  # Lower temp for code generation
                },
                timeout=300  # 5 min timeout for complex code
            )
            response.raise_for_status()
            data = response.json()
...
Write the complete document now:"""
        ...
        # Use routed endpoint for task-aware model selection (Qwen for code)
        result = self._call_llm_routed(prompt, max_tokens=4000)
    ...
    def _read_instruction_file(self, content: str) -> str:
        ...
        - "Details in /path/to/file.md"

        KB: KB-JR-EXECUTOR-INSTR-001 (Jan 24, 2026)
        """
        patterns = [
            r'[Ss]ee\s+(/\S+\.md)',
            ...
            if match:
                ...
                try:
                    if os.path.exists(file_path):
                        ...
                        print(f"[{self.agent_id}] Instruction file not found: {file_path}")
                except Exception as e:
    ...
    def _execute_code_task(self, task: dict) -> Tuple[bool, str]:
        ...
        # Detect language from instruction content (preferred) or task
        language = self._detect_language(instruction_content if instruction_content else task_content)

        # Get RAG context from codebase
        rag_context = self._get_code_rag_context(task_content)
...
You are given a CODE TEMPLATE below. Your job is to IMPLEMENT this template.
Do NOT create new code from scratch. Do NOT use different frameworks.
PRESERVE the template's structure, imports, and patterns.
Only fill in gaps marked with TODO, placeholders, or "...".
...
2. PRESERVE all imports from the template
3. PRESERVE function/class names from the template
4. PRESERVE the framework (FastAPI stays FastAPI, Next.js stays Next.js)
5. Fill in logic gaps while keeping template structure
6. NO markdown, NO backticks, NO explanations
7. Start with the first line of code (imports)

FARA CORRECTION RULES:
{fara_rules}

CODEBASE PATTERNS (for consistency):
{rag_context}

Implement the template above. Output the complete {language} code now:"""
        else:
            # GENERATION PROMPT - original behavior for tasks without templates
...
Generate the {language} code now. Output ONLY code, starting immediately:"""
        ...
        # Log prompt size for optimization research (KB-JR-COMPLEX-001)
        prompt_chars = len(prompt)
        prompt_tokens_est = prompt_chars // 4  # rough estimate
        print(f"[{self.agent_id}] Code prompt size: {prompt_chars} chars (~{prompt_tokens_est} tokens)")
        ...
        print(f"[{self.agent_id}] LLM raw response (first 500 chars): {result[:500]}")

        # Clean the output - remove any markdown/explanation that slipped through
        clean_code = self._clean_code_output(result, language)

        # Debug: Log cleaned code (first 500 chars)
        print(f"[{self.agent_id}] Cleaned code (first 500 chars): {clean_code[:500]}")

        # Validate syntax
        is_valid, validation_msg = self._validate_code_syntax(clean_code, language)

        if not is_valid:
            # Log failure with prompt size for optimization research
            print(f"[{self.agent_id}] PROMPT_SIZE_FAIL: {prompt_chars} chars, error: {validation_msg[:50]}")
            # Record FARA mistake
    ...
    def _detect_language(self, content: str) -> str:
        ...
        # TypeScript/React/Next.js detection (check first - more specific)
        if any(x in content_lower for x in ['.tsx', '.ts', 'typescript', 'react', 'next.js', 'next/link', 'use client', 'usestate', 'useeffect']):
            return 'typescript'
        elif any(x in content for x in ["import Link from 'next", 'export default function', 'className=']):
            return 'typescript'
        # Python/FastAPI detection
        elif 'python' in content_lower or '.py' in content_lower or 'fastapi' in content_lower or 'from app.' in content_lower:
            return 'python'
...
'''
        ...
        for line in lines:
            ...
            if not in_code:
                if language == 'python' and (stripped.startswith('import ') or
                    ...
                    stripped.startswith("'use client'") or stripped.startswith('"use client"') or
                    stripped.startswith('//')):
                    in_code = True
    ...
    def _validate_code_syntax(self, code: str, language: str) -> Tuple[bool, str]:
        ...
        if language == 'python':
            ...
            # Check for common patterns that indicate valid JSX/TSX
            has_imports = 'import ' in code
            has_export = 'export ' in code or 'module.exports' in code
            ...
            if abs(open_braces - close_braces) > 2:  # Allow small imbalance for template literals
                return False, f"Unbalanced brackets: {open_braces} open vs {close_braces} close"

            if has_imports or has_export or has_function:
                return True, "TypeScript/JavaScript appears valid"
            else:
                return True, "TypeScript/JavaScript structure accepted"
    ...
    def run(self):
        ...
        while self.running:
            try:
                # Periodic cleanup of stale tasks (every 5 minutes)
