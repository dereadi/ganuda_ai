#!/usr/bin/env python3
"""
Phase 2.5 Regression Testing - Validate Direct Answer Improvements
Tests whether 25 surgical scenarios in Redux format fixed regression failures
...
# Paths - Phase 2.5 specific
PHASE1_MODEL_PATH = "/ganuda/cherokee_resonance_training/cherokee_resonance_v1"
PHASE21_LORA_PATH = "/ganuda/cherokee_resonance_training/phase25_surgical_lora"
RESULTS_PATH = "/ganuda/phase23_regression_results.json"
LOG_PATH = "/ganuda/phase23_regression_testing.log"
...
log_progress("="*80)
log_progress("🦅 PHASE 2.5 REGRESSION TESTING")
log_progress("="*80)
log_progress("")
log_progress("Testing whether 25 surgical scenarios in Redux format fixed regression failures")
log_progress("Baseline: Phase 2 Redux (60% pass rate)")
log_progress("Target: Phase 2.5 (≥80% pass rate)")
log_progress("")
...
log_progress("📚 Loading Phase 2.5 model...")
...
def generate_response(prompt, max_tokens=200):
    """Generate response from Phase 2.5 model"""
...
log_progress(f"  Phase 2.5:     {pass_rate:.1%} pass rate ({passed_tests}/{total_tests} tests)")
...
if pass_rate >= 0.80 and avg_quality >= 7.0:
    ...
    log_progress("   Phase 2.5 achieved ≥80% pass rate with high quality")
    log_progress("   Direct answer scenarios successfully fixed regression failures")
    log_progress("")
    log_progress("Next steps:")
    log_progress("  1. ✅ Phase 2.5 regression validated")
    ...
    log_progress("   Phase 2.5 achieved ≥80% pass rate")
    ...
    log_progress(f"   Phase 2.5 improved from {phase2_redux_pass_rate:.0%} to {pass_rate:.1%}")
    ...
    log_progress("  - Consider Phase 2.5 iteration")

else:
    log_progress("❌ REGRESSION DETECTED")
    log_progress(f"   Phase 2.5 pass rate ({pass_rate:.1%}) ≤ Phase 2 Redux (60%)")
