...
def ask_council(context_text):
    ...
    prompt = (
        "You are the Cherokee AI Federation Council. Below is the current state "
        "of our federation — recent failures, low-confidence votes, open work items, "
        "and known research papers in our backlog.\n\n"
        "Based on this context, identify the 3 most valuable things we should "
        "research RIGHT NOW. These should directly help our cluster solve real "
        "problems, not just be interesting papers.\n\n"
        "For each item, provide:\n"
        "1. A specific search query for arxiv or GitHub\n"
        ...
        '"query", "source", "rationale", "specialist" '
...
def summarize_result(result_text):
    ...
    prompt = (
        "Summarize this for the Cherokee AI Federation council. "
        "How could this help our cluster? Be specific and brief (2-3 sentences max).\n\n"
        f"{result_text}"
    )
    ...
    for i, item in enumerate(council_items):
        ...
        try:
            if source == "github":
                ...
                time.sleep(3)  # Be kind to arxiv rate limits
                for r in raw_results:
            ...
            print(f"  [WARN] Research failed for query '{query}': {e}")
            continue
